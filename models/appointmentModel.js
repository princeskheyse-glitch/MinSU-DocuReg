/*
    MinSU DocuReg - Appointment Model (Firestore)
...
const col = () => db.collection(Collections.APPOINTMENTS);

const splitConditions = (conditions) => {
  const keys = Object.keys(conditions);
  if (keys.length === 0) return { fsField: null, fsValue: null, jsFilters: {} };
  const priority = ['campusId', 'studentId', 'documentRequestId', 'status'];
...
export const Appointment = {
  async create(data) {
    ...
    const now = new Date();
    const appt = {
      documentRequestId: data.documentRequestId, studentId: data.studentId,
      campusId: data.campusId || null, appointmentDate: data.appointmentDate,
      appointmentTime: data.appointmentTime, status: data.status || 'scheduled',
      notes: data.notes || null, registrarNotes: data.registrarNotes || null,
      createdAt: now, updatedAt: now
    };
    await col().doc(id).set(appt);
    return { id, ...appt };
  },
  ...
  async findAll(conditions = {}, opts = {}) {
    ...
    docs = sortDocs(docs, opts.orderBy || 'appointmentDate', opts.orderDir || 'asc');
