/*
    MinSU DocuReg - Document Request Model (Firestore)
...
const splitConditions = (conditions) => {
  ...
  const priority = ['campusId', 'studentId', 'status', 'documentType'];
...
export const DocumentRequest = {
  async create(data) {
    ...
    const now = new Date();
    const req = {
      studentId: data.studentId, campusId: data.campusId || null,
      documentType: data.documentType, purpose: data.purpose,
      quantity: data.quantity || 1, status: data.status || 'pending',
      notes: data.notes || null, rejectionReason: data.rejectionReason || null,
      processedBy: data.processedBy || null, completedAt: data.completedAt || null,
      createdAt: now, updatedAt: now
    };
    await col().doc(id).set(req);
    return { id, ...req };
  },
